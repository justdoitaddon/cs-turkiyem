...
class XhamsterProvider: MainAPI() {
    ...
    // Hàm phụ trợ parse JSON (No changes needed here, uses unified InitialsJson)
     private fun getInitialsJson(html: String): InitialsJson? {
        return try {
            ...
            println("getInitialsJson failed: ${e.message}")
            e.printStackTrace()
            null
        }
    }
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse? {
        ...
        println("getMainPage started for page $page")
        val document = try { app.get("$mainUrl/").document } catch (e: Exception) { println("Failed to fetch main page: ${e.message}"); return null }
        ...
        // Ưu tiên parse JSON
        if (initialData != null) {
            println("getMainPage: Parsed initialData JSON.")
            ...
            if (!items.isNullOrEmpty()) {
                ...
                println("getMainPage: Mapped ${items.size} items from JSON.")
            } else {
                 println("getMainPage: JSON items list is null or empty after mapping.")
                 items = null // Reset items để thử fallback
            }
        } else { println("getMainPage: Failed to parse initialData JSON.") }

        // Fallback về HTML nếu JSON thất bại hoặc không có item
        if (items.isNullOrEmpty()) {
            println("getMainPage: Falling back to HTML parsing.")
            items = document.select("div.mobile-video-thumb").mapNotNull { element ->
             ...
             if (items.isNotEmpty()) {
                 ...
                 println("getMainPage: Mapped ${items.size} items from HTML fallback.")
             } else {
                  println("getMainPage: HTML fallback also yielded no items.")
             }
        }

        return if (!items.isNullOrEmpty()) {
             println("getMainPage: Successfully loaded ${items.size} items using $listTitle logic.")
             ...
             println("getMainPage: No items found from JSON or HTML.")
             null
        }
    }
    ...
    override suspend fun search(query: String): List<SearchResponse>? {
        ...
        println("Search started for query '$query' at URL: $searchUrl")
        val document = try {
            app.get(searchUrl).document
        } catch (e: Exception) {
            println("Failed to fetch search page: ${e.message}")
            e.printStackTrace()
            return null
        }
        ...
        if (initialData != null) {
             println("Search: Parsed initialData JSON.")
             ...
             println("Search: Mapped ${results?.size ?: 0} items from JSON.")
        } else {
            println("Search: Failed to parse initialData JSON.")
        }

        // **Fallback về HTML nếu JSON thất bại hoặc không có item**
        if (results.isNullOrEmpty()) {
            println("Search: JSON results list is null or empty. Falling back to HTML parsing.")
            results = document.select("div.mobile-video-thumb").mapNotNull { element ->
                parseVideoItem(element)
            }
            println("Search: Mapped ${results.size} items from HTML fallback.")
        }

        return if (!results.isNullOrEmpty()) {
             println("Search: Found ${results.size} results for query '$query'.")
             results
        } else {
             println("Search: No results found for query '$query'.")
             null
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val document = try { app.get(url).document } catch (e: Exception) { println("Failed to load URL $url: ${e.message}"); return null }
        ...
        val title = initialData?.videoEntity?.title
            ...
            ?: run { println("Could not find title for $url"); return null }
        ...
        try {
            ...
            println("Found ${recommendations?.size ?: 0} recommendations using HTML selector '$relatedItemsSelector'.")

            // If the first selector failed, maybe try a broader one (less reliable)
            if (recommendations == null) {
                 println("Primary HTML selector for recommendations failed, trying broader selector.")
                 recommendations = document.select("ul.thumb-list div.mobile-video-thumb") // Broader guess
                    ...
                    .ifEmpty { null }
                 println("Found ${recommendations?.size ?: 0} recommendations using broader HTML selector.")
            }

        } catch (e: Exception) {
            println("Error parsing recommendations from HTML: ${e.message}")
            e.printStackTrace()
    ...
    override suspend fun loadLinks(
        ...
        // ... (Code giữ nguyên) ...
         println("LoadLinks started for: $data")
        val document = try { app.get(data).document } catch (e: Exception) { println("Failed to get document for loadLinks: ${e.message}"); return false }
        val initialData = getInitialsJson(document.html()) ?: run { println("Failed to parse JSON for loadLinks."); return false }
        ...
        } ?: println("No HLS source found in JSON.")
        ...
        sources?.standard?.h264?.forEach { qualitySource ->
            ...
            if (qualityLabel != null && videoUrl != null) {
                ...
                val quality = qualityLabel.replace("p", "").toIntOrNull() ?: Qualities.Unknown.value
                 println("Found MP4 link: $qualityLabel - $fixedVideoUrl")
            ...
            } else { println("MP4 source item missing quality or url: $qualitySource") }
        } ?: println("No Standard H264 sources found in JSON.")
        ...
        initialData.xplayerSettings?.subtitles?.tracks?.forEach { track ->
            ...
            if (subUrl != null) {
                ...
                println("Found subtitle: Lang=$lang, URL=$fixedSubUrl")
                try { subtitleCallback( SubtitleFile( lang = lang, url = fixedSubUrl )) } catch (e: Exception) { e.printStackTrace() }
            } else { println("Subtitle track missing VTT url: $track") }
        } ?: println("No subtitle tracks found in JSON.")

        if (!foundLinks) println("No video links (M3U8 or MP4) were found.")
        return foundLinks
    }
